...
Iterable<EmbedFieldBuilder> getMediaInfoEmbedFields(Iterable<MediaStream> mediaStreams) sync* {
  for (final mediaStream in mediaStreams) {
    ...
    yield EmbedFieldBuilder(name: "Media Info (${mediaStream.type!.name})", value: "$trackTitle ($bitrate Mbps)", isInline: true);
  }
}
...
EmbedBuilder? buildSessionEmbed(SessionInfo sessionInfo, JellyfinClientWrapper client) {
  ...
  if (nowPlayingItem.type == BaseItemKind.episode) {
    return EmbedBuilder(
      author: EmbedAuthorBuilder(name: nowPlayingItem.seriesName!, iconUrl: client.getItemPrimaryImage(nowPlayingItem.seriesId!)),
  ...
  children: [
    ChatCommand(
      ...
      id("jellyfin-current-sessions", (ChatContext context,
          [@Description("Name of instance. Default selected if not provided")
          @UseConverter(jellyfinConfigConverter)
          JellyfinConfig? config]) async {
        final client = await JellyfinModule.instance.getClient(JellyfinIdentificationModel(context.guild!.id, config?.name));
        if (client == null) {
    ...
    ChatCommand(
      ...
      id("jellyfin-transfer-config", (
        ...
        @Description("Guild to copy to") Snowflake targetGuildId, [
        ...
        final newConfig = await JellyfinConfigRepository.instance.createJellyfinConfig(
            configName ?? config.name, config.basePath, config.token, copyDefaultFlag && config.isDefault, targetGuildId);

        context.respond(MessageBuilder(content: 'Copied config: "${newConfig.name}" to guild: "${newConfig.guildId}"'));
      }),
    ),
    ChatCommand(
      "remove-config",
      "Removes config from current guild",
      id("jellyfin-remove-config",
          (ChatContext context, @Description("Name of instance") @UseConverter(jellyfinConfigConverter) JellyfinConfig config) async {
