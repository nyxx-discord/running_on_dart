...
final jellyfin = ChatGroup(
  ...
  checks: [jellyfinFeatureEnabledCheck],
  children: [
    ChatGroup(
      "wizarr",
      ...
      children: [
        ChatCommand(
          ...
          id("jellyfin-wizarr-redeem-invitation", (
        ...
        ChatCommand(
          ...
          id("jellyfin-wizarr-create-invitation", (
            InteractionChatContext context, [
            ...
            final firstModal = await context.getModal(
              title: "Create Wizarr invitation",
              components: [
                TextInputBuilder(
                  customId: "code",
            ...
            final message = await context.respond(
              MessageBuilder(
                content: "Click to open second modal and continue",
                components: [
                  ActionRowBuilder(
                    components: [
                      ButtonBuilder.primary(
                        customId: ComponentId.generate(allowedUser: context.user.id).toString(),
                        label: 'Open modal',
                      ),
                    ],
                  ),
            ...
            final secondModal = await context.getModal(
              title: 'Create Wizarr invitation',
              components: [
                TextInputBuilder(
            ...
            final accountDuration = getDurationFromStringOrDefault(
              valueOrNullIfNotDefault(secondModal['account_duration']),
              Duration(days: 1),
            );
            final expiresDuration = getDurationFromStringOrDefault(
              valueOrNullIfNotDefault(firstModal['expiration']),
              null,
            );
            ...
            if (result) {
              ...
              if (user != null) {
                ...
                (await user.manager.createDm(user.id)).sendMessage(
                  getWizarrRedeemInvitationMessageBuilder(
                    wizarrClient,
                    code,
                    user.id,
            ...
            return context.respond(
              MessageBuilder(content: 'Cannot create invitation. Contact administrator.'),
              level: ResponseLevel.private,
            );
          }),
        ),
    ...
    ChatGroup(
      "sonarr",
      "Sonarr related commands",
      children: [
        ChatCommand(
          "calendar",
          "Show upcoming episodes",
          id("jellyfin-sonarr-calendar", (
            ChatContext context, [
            @Description("Instance to use. Default selected if not provided") JellyfinConfigUser? config,
          ]) async {
    ...
    ChatCommand(
      ...
      id('jellyfin-search', (
        ChatContext context,
        @Description("Term to search jellyfin for") String searchTerm, [
        ...
        final resultsWithoutEpisodes = await client.searchItems(
          searchTerm,
    ...
    ChatCommand(
      ...
      id('jellyfin-info', (ChatContext context) async {
        ...
        final embeds = configs.map(
          (config) => EmbedBuilder(
            title: "${config.instanceName} (${config.instanceBaseBath})",
            fields: [
      ...
      children: [
        ChatCommand(
          ...
          id("jellyfin-settings-new-instance", (InteractionChatContext context) async {
            ...
            final message = await context.respond(
              MessageBuilder(
                content: "Click to open second modal",
                components: [
                  ActionRowBuilder(
                    components: [
                      ButtonBuilder.primary(
            ...
            final secondModalResponse = await context.getModal(
              title: "New Instance Configuration Pt. 2",
              components: [
          ...
          checks: [jellyfinFeatureCreateInstanceCommandCheck],
        ),
        ChatCommand(
          "edit-instance",
          "Edit jellyfin instance",
          id('jellyfin-settings-edit-instance', (
            InteractionChatContext context,
            @Description("Instance to use. Default selected if not provided") JellyfinConfig config,
          ) async {
            final modalResponse = await context.getModal(
              title: "Jellyfin Instance Edit Pt. 1",
              components: [
                TextInputBuilder(
                  customId: "base_url",
                  style: TextInputStyle.short,
            ...
            final secondModalResponse = await context.getModal(
              title: "Jellyfin Instance Edit Pt. 2",
              components: [
                TextInputBuilder(
        ...
        ChatCommand(
          ...
          id("jellyfin-settings-remove-config", (
            ChatContext context,
            @Description("Instance to use. Default selected if not provided") JellyfinConfigUser config,
          ) async {
            final client = await getJellyfinClient(config, context);
        ...
        ChatCommand(
          ...
          id('jellyfin-settings-ping', (
            ...
            return context.respond(
              MessageBuilder(content: 'Latency to jellyfin instance "${config.name}": ${stopwatchResult}ms'),
            );
          }),
        ),
      ],
    ...
    ChatGroup(
      "util",
      "Util commands for jellyfin",
      children: [
        ChatCommand(
          "complete-refresh",
          "Do a complete refresh of jellyfin instance content",
          id("jellyfin-util-complete-refresh", (
            ChatContext context, [
            @Description("Instance to use. Default selected if not provided") JellyfinConfigUser? config,
          ]) async {
            final client = await getJellyfinClient(config, context);
              ...
              tasks: [
                Task(
                  ...
                  updateCallback: () async {
                    final scheduledTask = (await client.getScheduledTasks()).firstWhereOrNull(
                      (taskInfo) => taskInfo.id == scanMediaLibraryTask.id,
                    );
                ...
                if (subtitleExtractTask != null)
                  Task(
                    ...
                    updateCallback: () async {
                      final scheduledTask = (await client.getScheduledTasks()).firstWhereOrNull(
                        (taskInfo) => taskInfo.id == subtitleExtractTask.id,
                      );
                      if (scheduledTask == null || scheduledTask.state == TaskState.idle) {
                        return (true, null);
                      }
