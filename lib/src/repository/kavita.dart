...
class KavitaRepository {
  ...
  Future<KavitaConfig?> find(String name, String parentId) async {
    final query =
        SelectQuery.selectAll(KavitaConfig.tableName)
  ...
  Future<KavitaUserConfig?> findUserConfigForConfig(String userId, int configId) async {
    final query =
        SelectQuery.selectAll(KavitaUserConfig.tableName)
  ...
  Future<KavitaUserConfig?> findUserConfig(String configName, String parentId, String userId) async {
    final query =
        SelectQuery.selectAll(KavitaConfig.tableName, alias: "c")
          ..select("uc.*")
    ...
    final result = await _database.executeQuery(
      query,
      parameters: {'configName': configName, 'parentId': parentId, 'userId': userId},
    );
  ...
  Future<KavitaConfig> saveConfig(KavitaConfig config) async {
    final query =
        InsertQuery(KavitaConfig.tableName)
          ..addNamedInsert('name')
