...
class GuildSettingsService {
  ...
  late final Future<List<GuildSetting<dynamic>>> settings =
      DatabaseService.instance.fetchSettings().then((value) => value.toList());

  GuildSettingsService._();

  /// Get the data for a setting in a specific guild, if any.
  Future<GuildSetting<T>?> getSetting<T>(
      Setting<T> setting, Snowflake guildId) async {
    final result = (await settings)
        .where((s) => s.setting == setting && s.guildId == guildId);
  ...
  Future<bool> isEnabled<T>(Setting<T> setting, Snowflake guildId) async =>
      await getSetting(setting, guildId) != null;
  ...
  Future<void> enable<T>(GuildSetting<T> setting) async {
    ...
    (await settings).removeWhere(
        (s) => s.setting == setting.setting && s.guildId == setting.guildId);
